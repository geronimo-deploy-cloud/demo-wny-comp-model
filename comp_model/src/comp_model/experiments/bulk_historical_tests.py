"""Bulk historical model analysis for comp_model.
...
Usage:
    python -m comp_model.bulk_historical_tests
    python -m comp_model.bulk_historical_tests --output-dir ./analysis_output
    python -m comp_model.bulk_historical_tests --max-samples 500
"""
...
from comp_model.sdk.data_sources import training_rochester
...
def load_and_score(max_samples: int | None = None) -> tuple[pd.DataFrame, pd.DataFrame, np.ndarray, np.ndarray]:
    ...
    from comp_model.sdk.data_sources import training_macro
...
def main():
    parser = argparse.ArgumentParser(
        description="Bulk historical analysis of comp_model on Rochester data."
    )
    parser.add_argument(
